...
def main() -> int:
    ...
    cumulative_promotions = {
        ...
        **{f"AUT-{number:03d}": "VERIFIED" for number in range(1, 11)},
        ...
        "ANA-011": "VERIFIED",
        "ANA-012": "VERIFIED",
        **{f"IMP-{number:03d}": "VERIFIED" for number in range(1, 11)},
        **{f"EXP-{number:03d}": "VERIFIED" for number in range(1, 5)},
        **{f"BKP-{number:03d}": "VERIFIED" for number in range(1, 8)},
        "SYS-003": "VERIFIED",
        **{f"RST-{number:03d}": "VERIFIED" for number in range(1, 8)},
        "CLR-002": "VERIFIED",
    }
    if len(screen_coverage) != 215 or any(
        ...
        fail("screen coverage contains a promotion outside the cumulative P31 scope")
