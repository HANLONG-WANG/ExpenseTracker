...
def validate_sources(sources: dict[str, str] | None = None) -> list[str]:
    ...
    require_tokens(errors, store + adapter, "encrypted revisioned persistence", (
        "LedgerDatabaseOperationAccess", "databaseAccess.withCurrentDatabase", "inLedgerTransaction",
        "expectedRowVersion", "AnalyticsError.RevisionConflict",
        "saveReport", "copyReport", "saveDashboard", "saveAnomalyRule", "anomalyFindings", "forecast",
        "ReportDerivationPolicy.derive",
    ))
    if re.search(r"\b(?:SharedPreferences|DataStore|Room\.databaseBuilder|fallbackToDestructiveMigration)\b", store + adapter):
...
def validate_ledgers() -> list[str]:
    ...
    mapping = read("docs/初始开发文件存档/implementation/P26_CUSTOM_ANALYTICS_MAPPING.md") if (ROOT / "docs/初始开发文件存档/implementation/P26_CUSTOM_ANALYTICS_MAPPING.md").is_file() else ""
