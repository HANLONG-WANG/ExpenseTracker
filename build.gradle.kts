...
tasks.register<JavaExec>("detekt") {
    ...
    inputs.file(layout.projectDirectory.file("config/detekt/detekt-baseline.xml"))
    outputs.files(
    ...
    args(
        ...
        "--baseline",
        layout.projectDirectory.file("config/detekt/detekt-baseline.xml").asFile.absolutePath,
...
val validateP02Specs by tasks.registering(Exec::class) {
    ...
    inputs.files(
        ...
        fileTree("docs/初始开发文件存档/implementation") { include("*.csv", "*.md") },
        files(
            "docs/初始开发文件存档/UI设计稿与实现契约_v1.0/android_ledger_ui_tokens_v1.json",
            "docs/初始开发文件存档/UI设计稿与实现契约_v1.0/android_ledger_screen_contract_v1.yaml",
            "docs/初始开发文件存档/UI设计稿与实现契约_v1.0/UI需求追踪矩阵_v1.csv",
        ),
    )
}
...
val validateP36ReleaseDelivery by tasks.registering(Exec::class) {
    ...
    inputs.files(
        ...
        fileTree("docs/初始开发文件存档/implementation") { include("*.csv", "*.md") },
        fileTree("docs/初始开发文件存档/release") { include("*.md") },
        files(
            "NOTICE",
...
val validateP37InteractivePerformanceImplementation by tasks.registering(Exec::class) {
    group = "verification"
    description = "Validates P37 session ownership, interaction traces, fixed budgets and benchmark wiring without claiming device evidence."
    workingDir(layout.projectDirectory)
    commandLine("python3", "scripts/validate_p37_interactive_performance.py", "--implementation-only")
    environment("PYTHONDONTWRITEBYTECODE", "1")
    inputs.files(
        "quality/performance/p37_budgets.json",
        "scripts/validate_p37_interactive_performance.py",
        "scripts/generate_p37_interactive_performance_evidence.py",
        fileTree("app/src") { include("**/*") },
        fileTree("benchmark/src") { include("**/*") },
        fileTree("core/security/src/main") { include("**/*.kt") },
        fileTree("finance/data/src/main") { include("**/*.kt") },
        fileTree("build-logic/src/main") { include("**/*.kt") },
    )
}

val validateP37InteractivePerformanceFixtures by tasks.registering(Exec::class) {
    group = "verification"
    description = "Proves the P37 gate rejects reduced samples, relaxed latency, frame-only evidence, early fully-drawn reporting and Port-local gates."
    workingDir(layout.projectDirectory)
    commandLine("python3", "-m", "unittest", "scripts.tests.test_p37_interactive_performance_contracts", "-v")
    environment("PYTHONDONTWRITEBYTECODE", "1")
    inputs.files(
        "quality/performance/p37_budgets.json",
        "scripts/validate_p37_interactive_performance.py",
        "scripts/generate_p37_interactive_performance_evidence.py",
        "scripts/tests/test_p37_interactive_performance_contracts.py",
    )
}

val validateP37InteractivePerformanceEvidence by tasks.registering(Exec::class) {
    group = "verification"
    description = "Validates checked-in P37 API 28/API 36 percentiles, counters, sample counts and timeout-free results."
    workingDir(layout.projectDirectory)
    commandLine("python3", "scripts/validate_p37_interactive_performance.py")
    environment("PYTHONDONTWRITEBYTECODE", "1")
    inputs.files(
        "quality/performance/p37_budgets.json",
        "quality/performance/p37_results_api28.json",
        "quality/performance/p37_results_api36.json",
        "scripts/validate_p37_interactive_performance.py",
    )
}

tasks.register("p37HostCheck") {
    group = "verification"
    description = "Runs the complete inherited host graph plus P37 implementation, benchmark compilation and negative fixtures."
    dependsOn(
        "p36Check",
...
tasks.register("p37Api28DeviceCheck") {
    group = "verification"
    description = "Runs the inherited API 28 matrix plus P37 target-scale counter and 5-cold/30-warm interaction benchmarks."
    dependsOn("p36Api28ManagedDeviceCheck", ":benchmark:pixel2Api28BenchmarkAndroidTest")
}

tasks.register("p37Api36DeviceCheck") {
    group = "verification"
    description = "Runs the inherited API 36 matrix plus P37 target-scale counter and 5-cold/30-warm interaction benchmarks."
    dependsOn("p36Api36ManagedDeviceCheck")
}

tasks.register("p37DeviceCheck") {
    group = "verification"
    description = "Runs both P37 target emulator environments."
    dependsOn("p37Api28DeviceCheck", "p37Api36DeviceCheck")
}

fun registerP37EvidenceGenerationTask(
    name: String,
    apiLevel: Int,
    deviceTask: String,
) = tasks.register<Exec>(name) {
    group = "reporting"
    description = "Generates integrity-linked P37 API $apiLevel evidence from raw AndroidX samples and device counter logs."
    dependsOn(deviceTask)
    workingDir(layout.projectDirectory)
    commandLine("python3", "scripts/generate_p37_interactive_performance_evidence.py", "--api-level", apiLevel.toString())
    environment("PYTHONDONTWRITEBYTECODE", "1")
    inputs.files(
        "quality/performance/p37_budgets.json",
        "scripts/generate_p37_interactive_performance_evidence.py",
        fileTree("benchmark/build/outputs/androidTest-results/managedDevice") { include("**/*") },
        fileTree("finance/data/build/outputs/androidTest-results/managedDevice") { include("**/*") },
        fileTree("app/build/outputs/apk/benchmark") { include("*.apk") },
        fileTree("benchmark/build/outputs/apk/benchmark") { include("*.apk") },
    )
    outputs.file("quality/performance/p37_results_api$apiLevel.json")
}

registerP37EvidenceGenerationTask("generateP37Api28PerformanceEvidence", 28, "p37Api28DeviceCheck")
registerP37EvidenceGenerationTask("generateP37Api36PerformanceEvidence", 36, "p37Api36DeviceCheck")

tasks.register("p37Check") {
    group = "verification"
    description = "Runs P37 host acceptance and validates checked-in API 28/API 36 evidence."
    dependsOn("p37HostCheck", validateP37InteractivePerformanceEvidence)
}
