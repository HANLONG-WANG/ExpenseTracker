...
| ADR-007A | Controlled privacy purge is the sole physical-delete exception and only applies to a fully reversed, closed transaction chain | VERIFIED (`P05-E002`, `P07-E001`, `P07-E003`, `P15-E004`, `P31-E004`: typed eligibility plus same-transaction retention/net/dependency/operation/backup revalidation, maintenance lock, guarded fact/history/reference deletion, GC, PURGE commit, payload-free tombstone, idempotency and injected rollback pass on SQLCipher) | Purge eligibility, maintenance-lock, tombstone and merge tests |
...
| ADR-013 | Restore validates in a shadow directory before atomic exchange | VERIFIED (`P05-E001`, `P31-E001`—`P31-E003`: authenticated bounded materialization, registered migration, SQLCipher shadow projection rebuild, integrity/FK/Journal/subtype checks, safety snapshot, same-filesystem atomic exchange and exact rollback pass) | Device fault-injection tests |
| ADR-014 | Managed backups are logically full and physically incremental | VERIFIED (`P05-E001`, `P30-E001`, `P30-E002`, `P30-E004`: authenticated repository header and immutable complete manifests reference deduplicated SQLCipher chunks/attachments; retention and reference-only GC pass) | Repository retention/deduplication tests |
| ADR-015 | Same-book merge uses stable IDs and a commit graph | VERIFIED (`P05-E001`, `P05-E002`, `P07-E001`, `P31-E002`, `P31-E003`: closest common ancestor, stable-ID/content-generation comparison, explicit transaction forks, tombstone precedence and coordinator-owned two-parent shadow merge commit pass without timestamp ordering) | Three-way merge and conflict tests |
...
|---|---|---|---|
| INV-001 | Every Journal Entry has equal base-currency debits and credits. | Domain property + database audit + restore validation | VERIFIED (`P06-E001`, `P06-E002`, `P07-E001`, `P07-E003`, `P08-E003`, `P31-E003`: planner/SQL/post-commit audits plus restored-shadow and live Journal balance validation pass) |
...
| INV-003 | Every formal transaction has at most one category, project and goal. | Type/domain validation + import tests | VERIFIED (`P05-E001`, `P06-E001`, `P06-E002`, `P28-E002`, `P28-E005`, `P35-E005`: closed payload/single typed effects and 100,000-row staging/shadow import plus row-99,999 rollback cannot create a split formal transaction) |
| INV-004 | Ordinary expense/income has one Primary amount and no category split. | Domain property + UI/import contract | VERIFIED (`P06-E001`, `P06-E002`, `P13-E001`—`P13-E004`, `P28-E002`, `P28-E005`, `P35-E005`: exactly one USER_INPUT/ACCOUNT/BASE component is enforced by planner, complete form, import preparation and target-scale fault replay) |
...
| INV-030 | Purge tombstones win over old entity versions during merge restore. | Merge integration tests | VERIFIED (`P05-E001`, `P05-E002`, `P07-E001`, `P31-E002`—`P31-E004`, `P31-E006`: either-side generation-aware tombstone selection, payload-free persistence and UI non-resurrection pass) |
...
| INV-032 | Vault fields never enter FTS, audit snapshots, logs or telemetry. | Static/privacy/device audit | VERIFIED (`P05-E001`, `P05-E002`, `P07-E001`—`P07-E003`, `P29-E001`, `P32-E001`—`P32-E006`: separate SQLCipher ciphertext table, exact FTS/export allowlists, audit-snapshot/device sentinel scans, closed telemetry schemas, sanitized ACRA/upload scan and merged/unmerged semantics tests pass) |
| INV-033 | Failed import, restore or large batch leaves the main ledger unchanged. | Shadow-DB fault injection | VERIFIED (`P05-E001`, `P05-E002`, `P07-E001`, `P07-E003`, `P24-E003`, `P28-E002`, `P28-E005`, `P31-E003`, `P31-E004`: batch/import plus every restore exchange point, ENOSPC, process-death and purge checkpoint leave or restore the exact prior primary/key/artifact state) |
...
| INV-035 | Every cache depending on current transaction content carries a version. | Architecture/cache invalidation tests | VERIFIED (`P05-E001`, `P07-E001`, `P08-E001`, `P08-E003`, `P14-E004`, `P25-E003`, `P26-E003`, `P33-E003`, `P35-E001`, `P35-E002`, `P35-E006`: Schema v4's 15-row family generation gate covers all financial/analytics/widget families with local and valuation revisions; stale rows are withheld and deterministic rebuild repairs the family) |
...
| Trash retention and purge | Trash reversal yields zero current net effect; P31 repeats retention, every typed net, dependency, durable-operation and backup-reference check under maintenance in the coordinator transaction, removes the closed chain and queues orphan blobs before one PURGE commit/tombstone (`P15-E004`, `P31-E004`) |
...
## P29 ordinary export domain and encrypted application boundary

P29 completes ordinary export without changing Room Schema v2 and without creating any financial write path. Export reads the live ledger or a P26 prepared report through typed, bounded sources; portable backup remains a different closed format owned by P30+.

| Domain/schema surface | P29 realization |
|---|---|
| Closed descriptor | Content is current filter, full workbook or prepared report; the legal formats are CSV, XLSX, PDF and PNG, while `PORTABLE_BACKUP` is rejected. Scope, versions, local/valuation revision and the non-backup disclaimer are immutable output metadata (`P29-E001`, `P29-E002`) |
| Sensitive field authority | `ExportField` contains only ordinary allowlisted fields; complete card/account secrets, security codes, password/ciphertext and vault values have no representation. Card output is last-four only and both location E7 fields are paired opt-in/default-off (`P29-E001`, `P29-E005`) |
| Query boundary | Current-filter CSV reuses the P15 journal application keyset query; all 15 workbook sheets use read-only allowlisted SQLCipher adapters; report export uses the encrypted revisioned P26 snapshot. No feature, Worker or export data path writes a financial DAO (`P29-E001`, `P29-E005`) |
| Bounded writers | Commons CSV and FastExcel 0.20.2 request 256-row pages; Android `PdfDocument` keeps one active page; PNG retains a fixed 22-row display sample while scanning the complete source. The 100,000-row/large-document device suite stays within a 512 MiB heap class (`P29-E002`, `P29-E003`) |
| Durable operation | `background_operation` persists encrypted export parameters/progress/checkpoints; the SAF handle is separately encrypted and WorkManager/UIDT receives only `operationId`. Process recreation regenerates unpublished output or reconciles a readable `COMMITTING` result (`P29-E004`) |
| SAF publication | App-private temporary output streams to provider `.partial`, existing output moves to `.previous`, the complete new document is atomically named, and cancel/failure removes temporary material or restores the previous file. Space, permission and unavailable-provider outcomes are typed (`P29-E003`, `P29-E004`) |

## P30 encrypted backup domain and repository boundary

P30 implements backup over the existing encrypted Schema v2 ledger without creating a financial writer. Consistent SQLCipher shadow snapshots are obtained through the application-owned input boundary; the daily trigger observes successful `FinancialMutationCoordinator` commits and schedules an opaque operation rather than issuing a DAO mutation.

| Domain/schema surface | P30 realization |
|---|---|
| Repository identity and publication | A versioned authenticated repository header identifies the repository/book/schema/algorithm. Objects publish before an encrypted manifest; the SQLCipher catalog inserts one immutable COMPLETE snapshot only after every referenced object decrypts and hashes correctly. Exact replay is idempotent and conflicting metadata fails closed (`P30-E001`, `P30-E002`, `P30-E004`) |
| Logical-full/physical-incremental graph | Fixed 4 MiB SQLCipher chunks plus settings/history/attachment/Vault object kinds use SHA-256 identity and Streaming AEAD storage. Every manifest has an ordered complete reference set while unchanged physical objects are reused (`P30-E002`, `P30-E004`) |
| Retention and garbage collection | Default count is 30 with configurable count/age. Snapshot manifests and guarded immutable reference facts are removed first; only objects with no remaining snapshot reference in the same repository are deleted. Interrupted unpublished objects are eligible for the same collector (`P30-E001`, `P30-E002`, `P30-E004`) |
| Recovery key boundary | Repository keys have a Keystore device envelope and a separately salted/parameterized Argon2id recovery envelope. Password changes can affect future backups only or stream accessible historical manifest re-encryption. Optional Vault material is a recovery-wrapped encrypted Vault key plus ciphertext; background backup never receives PAN/CVC plaintext (`P30-E001`—`P30-E003`) |
| Portable and SAF boundary | The portable `.ledger-backup` is one ZIP64 stream protected by Tink Streaming AEAD and includes authenticated header/manifest/database/settings/attachments/history/optional Vault material. SAF uses persisted tree authority and recoverable provider temporary names with typed revocation/space/cancel outcomes (`P30-E002`, `P30-E005`) |
| Drive and durable operation | Google Identity requests only `drive.file`; Drive REST v3/OkHttp uses a repository-specific folder, persisted SQLCipher resumable session URL/offset, Content-Range/Range reconciliation and object checksum verification. Only the final current manifest makes a snapshot valid and publication retries do not duplicate it (`P30-E001`, `P30-E002`, `P30-E004`) |

## P31 restore, merge and controlled purge boundary

P31 consumes P30 backup artifacts without adding a database or alternate financial writer. Encrypted source material becomes plaintext only inside a scoped app-private restore directory, is copied/hash-checked with bounded buffers, and is destroyed after success, safe cancellation or recoverable failure.

| Domain/schema surface | P31 realization |
|---|---|
| Replacement validation | Recovery-restored key material opens a SQLCipher shadow; registered Room migrations, projection rebuild, integrity/FK/Journal/subtype/capability and book/base/head checks must pass before exchange (`P31-E001`—`P31-E003`) |
| Atomic live exchange | Pre-restore repository/quarantine safety snapshots plus database sidecar, key, settings, attachment and Vault stage/safety artifacts are governed by PREPARED/FINALIZED markers and same-filesystem atomic moves. Exchange/live validation/rollback is non-cancellable and process-recoverable (`P31-E003`) |
| Commit graph merge | The existing `book_commit_parent`, `entity_change`, `entity_revision`, `merge_session`, `merge_conflict` and `merge_resolution` records drive closest-ancestor three-way comparison. Missing incoming commits and the two-parent MERGE commit are inserted in one coordinator-owned shadow transaction (`P31-E002`, `P31-E003`) |
| Purge exception | The financial repository acquires maintenance and the narrow runtime purge guard, revalidates the fully reversed/closed chain, deletes immutable facts/history/reference edges, queues orphan blobs, appends PURGE/entity-change/receipt/tombstone and advances the head atomically (`P31-E004`) |
| Tombstone precedence | `purge_tombstone` stores only entity type/ID, purge commit/time and generation. Either-side tombstones supersede older/equal entity generations during merge and cannot be overridden by a user resolution (`P31-E002`, `P31-E004`, `P31-E006`) |
| Clear and recovery | Local clear destroys the primary/key plus app-owned backup/attachment/restore/import/shadow/checkpoint material after joining work, but never user SAF exports or Drive. Cloud deletion independently removes manifest/catalog visibility before reference-only object GC (`P31-E005`) |

## P32 Vault and privacy-diagnostics boundary

P32 adds no financial mutation path and no schema migration. Vault persistence is a narrow ciphertext-only application port over the existing `card_vault_secret` table. It never constructs a Journal, Posting, revision snapshot or projection, while payment-card identity remains owned by the existing reference-data/coordinator flow.

| Domain/schema surface | P32 realization |
|---|---|
| `card_vault_secret` | PAN, CVC, holder, expiry and custom sensitive fields are independently AEAD-encrypted with book/card/field/schema associated data. The SQLCipher adapter accepts only opaque ciphertext and stable card identity; real-device read/save/list/delete passes (`P32-E003`, `P32-E005`) |
| Vault key hierarchy | Vault DEK is wrapped by an authentication-validity-zero Keystore KEK and optionally by the independent recovery-password envelope. Restore rewrap creates a fresh device-auth KEK before normal use; app lock has no access to either primitive (`P32-E003`) |
| Plaintext lifetime | One-shot edit/reveal requests and bounded exposure handles zeroize on consume/close. Thirty-second expiry, background, leave, app lock and close clear values; PAN clipboard is sensitive/timed and there is no CVC clipboard function (`P32-E003`, `P32-E006`) |
| Diagnostics store | Fixed feature/crash records, separate rotating IDs and atomic bounded queues live under no-backup app storage. They are diagnostic Cache records, never ledger authority, backup content or financial facts (`P32-E002`, `P32-E004`) |
| ACRA/exit adapter | Only Android version and sanitized symbol frames enter the custom sender. Exception messages, business objects and arbitrary maps/text are unrepresentable; `ApplicationExitInfo` maps system reasons to closed codes (`P32-E001`, `P32-E002`, `P32-E004`) |
| Clear boundary | CLR-001 joins work, destroys the local book/key/Vault/diagnostic/app-owned artifacts and resets settings after a fresh authentication. It cannot reach SAF exports or Drive; CLR-002 retains separate cloud authority (`P31-E005`, `P32-E006`, `P32-E007`) |

## P33 widget projection and background-operation boundary

P33 adds no financial fact or writer. Primary Schema v3 non-destructively extends the four existing widget snapshot projections; the projection engine remains a synchronous derived-state consumer of committed facts. Glance itself opens SQLCipher only through a bounded read port and cannot query transactions/postings or trigger a rebuild.

| Domain/schema surface | P33 realization |
|---|---|
| Widget book snapshot | Local/valuation revision plus snapshot date, month/current-prior consumption, budget available/used, today available and current/prior core net assets support the frozen summary widgets without transaction rows (`P33-E001`—`P33-E003`) |
| Account/card/goal snapshots | Stable IDs, display labels and only the bounded balances/availability/statement/target values needed by selectable widgets are stored; no note, merchant, place, coordinate, attachment or Vault field exists (`P33-E001`, `P33-E003`) |
| Date-boundary rebuild | Ready-session and foreground transitions compare the saved local date and rebuild only the four widget projections inside one SQLCipher transaction; same-day calls are idempotent and Glance stays read-only (`P33-E003`) |
...
| SCHEMA-FAMILY-02 | Accounts and cards | `ledger_account`, `user_account`, `payment_card`, `card_vault_secret`, `account_balance_checkpoint` | VERIFIED (`P07-E001`—`P07-E003`, `P32-E003`, `P32-E005`: P32 adds the actual per-action authenticated AEAD producer and ciphertext-only SQLCipher application adapter, with no FTS/audit/export leakage) |
...
| PROJECTION-FAMILY-07 | Widgets | `widget_book_snapshot`, `widget_account_snapshot`, `widget_credit_snapshot`, `widget_goal_snapshot` | VERIFIED (`P08-E001`, `P08-E003`, `P14-E004`, `P33-E001`—`P33-E004`: primary Schema v3 supplies the complete bounded snapshot model; transaction-time and date-boundary rebuilds are versioned and the API 28/API 36 Glance path reads only these four tables) |
...
|---|---|---|---|
| BACKGROUND-OPERATION | Durable operation state | `background_operation`, `operation_checkpoint`; QUEUED → PREPARING → RUNNING → PAUSED/CANCEL_REQUESTED/FAILED_* → COMMITTING/ROLLING_BACK → SUCCEEDED | VERIFIED for import/export/backup/restore and P33 operation-center integration (`P05-E001`, `P05-E002`, `P07-E001`, `P28-E001`, `P28-E002`, `P28-E004`, `P29-E001`, `P29-E004`, `P30-E001`, `P30-E002`, `P30-E004`, `P31-E001`—`P31-E003`, `P33-E001`, `P33-E006`: encrypted parameters/checkpoints, newest-first status, bounded progress, safe cancel, notification deep link, crash recovery and cleanup pass) |
...
| BACKUP-RESTORE-METADATA | Backup/Drive/restore/merge | `backup_repository`, `backup_snapshot`, `backup_object`, `backup_snapshot_object`, `drive_upload_session`, `restore_record`, `merge_session`, `merge_conflict`, `merge_resolution` | VERIFIED (`P05-E001`, `P05-E002`, `P07-E001`, `P30-E001`, `P30-E002`, `P30-E004`, `P31-E001`—`P31-E003`: immutable backup graph, Drive resume, restore state, conflict resolution and final merge/replace audit run through SQLCipher and crash recovery) |
...
## P35 target-scale and Schema v4 result

P35 adds no new business entity or financial write route. Primary Schema v4 adds only `projection_family_state`, one bounded generation row for each of 15 authoritative projection families. Each row carries the current local revision and valuation revision. Normal coordinator-owned financial commits update affected projections and their family generations inside the same SQLCipher transaction; startup compares only the bounded family-generation set. Full row-count, foreign-key, Journal, projection and canonical-hash audits remain maintenance/restore operations and were not weakened.

The benchmark fixture lives exclusively in `app/src/benchmark`, uses batches of at most 1,000 rows, and cannot be packaged in debug/release. API 36 evidence executes 500,000 current transactions, 1,000,000 immutable revisions, 1,000,000 postings, 100,000 attachment associations, 50,000 files and 10,000 merchant/place rows with bounded keyset/FTS/report/R*Tree plans. API 28 and API 36 emulator regressions execute SQLCipher migrations, Keystore/Vault behavior, coordinator persistence and restore exchange. Exact provenance, budgets and measurements are in `P35_PERFORMANCE_FAULT_SECURITY_AUDIT.md`.

## Mandatory test and release quality gates

| Gate | Frozen source | Status |
|---|---|---|
| Pure domain property suite for accounting, refunds, FX, loans, installments, budget, settlement, expressions and recurrence | Tech stack §16; architecture §21 | VERIFIED (`P03-E002`—`P03-E005`, `P05-E002`, `P06-E001`—`P06-E003`, `P16-E002`, `P17-E002`, `P18-E002`, `P20-E002`, `P21-E002`, `P22-E002`, `P23-E002`, `P35-E005`, `P35-E008`: all owning aggregate properties plus ten-year budget and final aggregate replay pass) |
| Room/SQLCipher schema, all migrations, FTS5, R*Tree, WAL plaintext and projection rebuild on device | Tech stack §§5,16; architecture §21 | VERIFIED (`P07-E001`—`P07-E004`, `P08-E003`, `P17-E003`, `P18-E003`, `P25-E003`, `P26-E003`, `P33-E003`, `P35-E002`, `P35-E006`: v1→v4 migration, target-scale indexed queries, bounded startup generations, WAL/checkpoint and deterministic stale-family rebuild pass on API 28/API 36 emulators) |
| Keystore, BiometricPrompt, SAF, location and foreground/UIDT behavior on Android emulators | Tech stack §16 | VERIFIED for the user's explicitly substituted P35 environment (`P09-E003`, `P09-E004`, `P10-E003`, `P10-E004`, `P28-E003`, `P28-E004`, `P29-E004`, `P29-E005`, `P30-E003`—`P30-E005`, `P31-E003`, `P31-E005`, `P31-E006`, `P32-E003`—`P32-E005`, `P35-E006`: API 28 PIN/enrolled emulator fingerprint and API 36 credential suites exercise the real Android platform APIs; this is not represented as physical-device evidence) |
| Failure injection for attachment, commits, Drive, storage, restore exchange, Keystore, biometrics, row 99,999 and projection versions | Architecture §21.3 | VERIFIED (`P08-E003`, `P09-E003`, `P09-E004`, `P10-E003`, `P28-E002`, `P28-E005`, `P30-E002`—`P30-E005`, `P31-E002`—`P31-E005`, `P35-E005`, `P35-E006`: the complete named matrix is re-executed and preserves main-ledger atomicity, cleanup and recovery) |
