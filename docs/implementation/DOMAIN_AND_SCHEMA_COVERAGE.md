...
Stage: P07
Status meaning: `NOT_STARTED`, `IN_PROGRESS`, `IMPLEMENTED`, `VERIFIED`, `BLOCKED`. P07 verifies encrypted physical Schema v1 and migration/device contracts; repository mapping, projection rebuild behavior, security-key adapters and feature behavior remain later scope.
...
| ADR-002 | SQLCipher primary database is the ledger's sole source of truth | IN_PROGRESS (`P07-E001`, `P07-E003`: encrypted sole-primary/staging topology and device proof; repository integration remains P08) | Device database and offline-first integration tests |
| ADR-003 | Current state + immutable revisions + immutable financial log | IN_PROGRESS (`P05-E001`, `P05-E002`, `P06-E001`, `P06-E002`, `P07-E001`—`P07-E003`: planner and append-only physical schema complete; repository transaction remains P08) | Domain and database contract tests |
| ADR-004 | No full event sourcing | IN_PROGRESS (`P05-E001`, `P07-E001`: normalized current pointers plus immutable facts/revisions physically separated) | Schema/API inspection |
| ADR-005 | Lightweight CQRS separates writes from query projections | IN_PROGRESS (`P05-E001`, `P05-E004`, `P07-E001`: complete separate physical projection families; rebuild/query behavior remains P08+) | Module/API and projection tests |
| ADR-006 | Every financial write passes through `FinancialMutationCoordinator` | IN_PROGRESS (`P05-E002`, `P05-E003`, `P06-E002`, `P06-E003`, `P07-E004`: canonical/idempotent coordinator and static call-site gate remain intact; Room repository integration remains P08) | Static call-site rule and integration tests |
| ADR-007 | Journal and Posting are append-only in ordinary operations | IN_PROGRESS (`P05-E001`, `P05-E002`, `P06-E001`, `P06-E002`, `P07-E001`—`P07-E003`: immutable triggers and reversal uniqueness verified; Room DAO integration remains P08) | DAO constraints and mutation tests |
...
| ADR-008 | Editing reverses old effects and appends replacements | IN_PROGRESS (`P05-E001`, `P06-E001`, `P06-E002`, `P07-E001`: immutable revision/fact/reversal schema complete; persistence execution remains P08) | Property and integration tests |
| ADR-009 | Core financial projections update in the same database transaction | IN_PROGRESS (`P05-E001`, `P05-E002`, `P06-E002`, `P07-E001`: all core projections share the primary DB and revision columns; failure-injection remains P08) | Failure-injection and rollback tests |
...
|---|---|---|---|
| INV-001 | Every Journal Entry has equal base-currency debits and credits. | Domain property + database audit + restore validation | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`, `P07-E003`: planner properties plus SQL `CHECK`/audit view/device integrity; restore validation remains P31) |
| INV-002 | Every Posting currency matches its LedgerAccount currency. | Planner property + database audit | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`, `P07-E003`: typed planner plus SQL cross-row currency trigger/device audit; repository mapping remains P08) |
...
| INV-005 | Every current transaction references one complete, self-consistent current revision. | Database integrity audit | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`, `P07-E003`: pointer ownership trigger, subtype audit view and device integrity; repository atomicity remains P08) |
| INV-006 | Old revisions, Journals and Effects are not changed during ordinary operations. | DAO/static rule + mutation tests | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`—`P07-E003`: exact reversal plus 63 immutable-table update/delete guards; DAO integration remains P08) |
| INV-007 | Each APPLY Entry is reversed at most once. | Unique constraint + property tests | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`, `P07-E003`: planner rejection plus partial unique reversal indexes; repository integration remains P08) |
...
| INV-016 | Current FX rates cannot change historical base-currency amounts. | Historical-regression tests | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`: immutable RevisionAmount/FxRateSnapshot/Posting plus rule-version schema; repository round trip remains P08) |
...
| INV-031 | Every core projection aligns to the same `localRevision`. | Atomicity/failure-injection audit | IN_PROGRESS (`P06-E001`, `P06-E002`, `P07-E001`: every core projection carries the required revision and an audit view exists; persisted atomicity remains P08) |
...
| INV-035 | Every cache depending on current transaction content carries a version. | Architecture/cache invalidation tests | IN_PROGRESS (`P05-E001`, `P05-E002`, `P07-E001`: all transaction-dependent projection/cache tables carry revision columns; invalidation behavior remains P08+) |
...
Source: domain/schema document §25. The inventory names every logical table, including the 11 typed transaction-detail tables. P07 verifies all 12 physical Schema v1 families; repository mapping and transactional behavior remain P08+.
...
|---|---|---|---|
| PROJECTION-FAMILY-01 | Current transaction | `current_transaction_projection` | IN_PROGRESS (`P07-E001`, `P07-E003`: physical schema/view complete; P08 rebuild/query behavior remains) |
| PROJECTION-FAMILY-02 | Account | `account_balance_current`, `account_valuation_current`, `account_balance_daily` | IN_PROGRESS (`P07-E001`, `P07-E003`: physical schema/window view complete; P08 rebuild/query behavior remains) |
| PROJECTION-FAMILY-03 | Budget and planning | `budget_usage_projection`, `budget_future_reservation`, `project_usage_projection`, `goal_balance_projection`, `budget_rollover`, `refund_status_projection` | IN_PROGRESS (`P07-E001`: physical schema complete; owning rebuild algorithms remain) |
| PROJECTION-FAMILY-04 | Liabilities | `credit_statement_projection`, `credit_account_projection`, `installment_progress_projection`, `loan_progress_projection`, `loan_future_cashflow_projection`, `loan_simulation_item` | IN_PROGRESS (`P07-E001`: physical schema complete; owning rebuild algorithms remain) |
| PROJECTION-FAMILY-05 | Settlement | `settlement_position_projection` | IN_PROGRESS (`P07-E001`: physical schema complete; P08/P22 rebuild behavior remains) |
...
| PROJECTION-FAMILY-07 | Widgets | `widget_book_snapshot`, `widget_account_snapshot`, `widget_credit_snapshot`, `widget_goal_snapshot` | IN_PROGRESS (`P07-E001`: physical snapshots complete; P33 update/runtime behavior remains) |
| INDEX-FAMILY-01 | Search | `transaction_fts`; current effective transactions only; no vault/account/location-sensitive fields | VERIFIED (`P07-E001`—`P07-E003`: exact allowlist, trigram FTS5 device query and encrypted side-file scan) |
| INDEX-FAMILY-02 | Geography | `location_rtree`, `place_rtree`; R*Tree bounding candidates followed by precise Kotlin distance | VERIFIED (`P07-E001`—`P07-E003`: both virtual tables and device bounding query) |
...
| Room/SQLCipher schema, all migrations, FTS5, R*Tree, WAL plaintext and projection rebuild on device | Tech stack §§5,16; architecture §21 | IN_PROGRESS (`P07-E001`—`P07-E004`: first-version schema/create-reopen, explicit empty predecessor registries, FTS5/R*Tree/JSON/window, integrity/FK, WAL settings and encrypted side-file scan are verified on API 36; projection rebuild behavior remains P08) |
