...
private fun BackupSettings(state: BackupFlowUiState, actions: BackupFlowActions, modifier: Modifier) {
    LazyColumn(modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.sm)) {
        ...
        item { LedgerButton(stringResource(R.string.backup_save_settings), actions.onSaveSettings, Modifier.fillMaxWidth()) }
    }
}
...
private fun BackupHistory(state: BackupFlowUiState, actions: BackupFlowActions, modifier: Modifier) {
    LazyColumn(modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.sm)) {
        ...
        if (state.snapshots.isEmpty()) item { LedgerBanner(stringResource(R.string.backup_history_empty), LedgerBannerVariant.INFO) }
        items(state.snapshots, key = BackupSnapshotUi::snapshotId) { snapshot ->
            LedgerCard(Modifier.fillMaxWidth(), onClick = { actions.onSnapshotSelected(snapshot.snapshotId) }) {
                Column(Modifier.padding(LedgerTheme.spacing.sm)) {
                    ...
                    LedgerText(snapshot.logicalContent, LedgerTextRole.BODY)
                    LedgerText(stringResource(R.string.backup_snapshot_increment, snapshot.physicalIncrement), LedgerTextRole.SUPPORTING)
...
private fun BackupDetails(state: BackupFlowUiState, actions: BackupFlowActions, modifier: Modifier) {
    ...
    Column(modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.sm)) {
        ...
        SummaryCard(R.string.backup_contents, snapshot.logicalContent)
    ...
    LazyColumn(modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.sm)) {
        ...
        item {
            when (state.execution) {
                ...
                BackupExecutionPresentation.SUCCEEDED -> LedgerBanner(stringResource(R.string.backup_succeeded), LedgerBannerVariant.INFO)
            }
        }
