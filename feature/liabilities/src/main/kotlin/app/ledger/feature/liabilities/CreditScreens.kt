    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_PAYMENT)) {
        ...
        item {
            FormSection(stringResource(R.string.credit_payment_accounts)) {
                SelectionRow(account.name, actions.onNextPaymentAccount)
                SelectionRow(paymentAccountName(state), actions.onNextPaymentAccount)
            }
        }
        item {
            LedgerTextField(
        ...
        item { LedgerTextField(state.draft.date, { actions.onFieldChanged(CreditField.DATE, it) }, stringResource(R.string.credit_payment_date), Modifier.fillMaxWidth(), required = true) }
...
private fun CreditAccountDetail(state: CreditFeatureState, actions: CreditActions) {
    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_ACCOUNT_DETAIL)) {
        ...
        item { LedgerButton(stringResource(R.string.credit_statements), { actions.onNavigate("CRD-003", account.id) }, Modifier.fillMaxWidth(), LedgerButtonVariant.SECONDARY) }
...
private fun CreditProfileEditor(state: CreditFeatureState, actions: CreditActions) {
    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_PROFILE)) {
        ...
        item { LedgerTextField(state.draft.dueDay, { actions.onFieldChanged(CreditField.DUE_DAY, it) }, stringResource(R.string.credit_due_day), Modifier.fillMaxWidth(), keyboardType = KeyboardType.Number) }
        item { LedgerTextField(state.draft.zoneId, { actions.onFieldChanged(CreditField.ZONE, it) }, stringResource(R.string.credit_statement_zone), Modifier.fillMaxWidth(), required = true) }
        ...
        item { LedgerTextField(state.draft.temporaryExpires, { actions.onFieldChanged(CreditField.TEMPORARY_EXPIRY, it) }, stringResource(R.string.credit_temporary_expiry), Modifier.fillMaxWidth()) }
...
private fun CreditStatementList(state: CreditFeatureState, actions: CreditActions) {
    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_STATEMENTS)) {
        items(account.statements, key = { it.id.toString() }) { statement -> StatementRow(statement, account, actions) }
        item { LedgerText(stringResource(R.string.credit_no_minimum_payment), LedgerTextRole.SUPPORTING) }
    }
}
...
private fun StatementRow(statement: CreditStatementView, account: CreditAccountView, actions: CreditActions) {
    LedgerCard(Modifier.fillMaxWidth(), onClick = { actions.onNavigate("CRD-004", statement.id) }) {
        Column(Modifier.padding(LedgerTheme.spacing.sm), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.xs)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                LedgerText(stringResource(R.string.credit_cycle, statement.cycleStart.toString(), statement.cycleEnd.toString()), LedgerTextRole.SECTION)
            ...
            LedgerText(stringResource(R.string.credit_due_date, statement.dueDate.toString()), LedgerTextRole.SUPPORTING)
            LedgerText(stringResource(R.string.credit_estimate_official, statement.estimatedAmountMinor, statement.officialAmountMinor?.toString() ?: stringResource(R.string.credit_not_recorded)), LedgerTextRole.SUPPORTING)
        }
    }
}
...
private fun OfficialStatementEditor(state: CreditFeatureState, actions: CreditActions) {
    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_OFFICIAL_STATEMENT)) {
        item { LedgerText(stringResource(R.string.credit_estimated_value, statement.estimatedAmountMinor), LedgerTextRole.BODY) }
    ...
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_ASSIGNMENT)) {
        ...
        item {
            FlowRow(horizontalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.xs)) {
                LedgerButton(stringResource(R.string.credit_previous_cycle), { actions.onAssignment(StatementAssignmentMode.PREVIOUS_CYCLE) }, variant = LedgerButtonVariant.SECONDARY)
                LedgerButton(stringResource(R.string.credit_current_cycle), { actions.onAssignment(StatementAssignmentMode.EXPLICIT_STATEMENT) })
                LedgerButton(stringResource(R.string.credit_next_cycle), { actions.onAssignment(StatementAssignmentMode.NEXT_CYCLE) }, variant = LedgerButtonVariant.SECONDARY)
            }
        }
...
private fun AutoPaymentEditor(state: CreditFeatureState, actions: CreditActions) {
    ...
    val eligible = state.presentation == CreditPresentation.ELIGIBLE
    ScreenList(Modifier.testTag(LedgerTestTags.CREDIT_AUTO_PAYMENT)) {
        item { LedgerToggleRow(stringResource(R.string.credit_auto_payment), eligible, actions.onToggleAutoPayment, supportingText = stringResource(R.string.credit_auto_mode_explanation)) }
        ...
        item { EligibilityRow(stringResource(R.string.credit_eligibility_payment_account), account.profile?.defaultPaymentAccountId != null) }
        item { EligibilityRow(stringResource(R.string.credit_eligibility_active), !account.archived) }
        item { EligibilityRow(stringResource(R.string.credit_eligibility_duplicate), true) }
        if (!eligible) item { LedgerBanner(stringResource(R.string.credit_candidate_fallback), LedgerBannerVariant.WARNING) }
        item { LedgerBanner(stringResource(R.string.credit_bookkeeping_disclaimer), LedgerBannerVariant.INFO) }
    }
}
    ...
    LedgerCard(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(LedgerTheme.spacing.sm), verticalArrangement = Arrangement.spacedBy(LedgerTheme.spacing.xs)) {
            ...
            account.statements.filter { it.remainingAmountMinor > 0L }.take(3).forEach { statement ->
                LedgerButton(
                    stringResource(R.string.credit_specific_statement, statement.dueDate.toString()),
