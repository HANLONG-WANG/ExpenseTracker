...
/** Encrypted-at-rest SAF handle storage. Routes, SavedState and operation parameters carry only the opaque id. */
class SecureImportSourceHandleStore(
    context: Context,
    private val keyProvider: DeviceLedgerKeyProvider,
) {
    private val directory = context.applicationContext.noBackupFilesDir.resolve("import_source_handles")
    ...
    fun save(bookId: StableId, handleId: StableId, persistedHandle: String) {
        ...
        val temporary = directory.resolve(".${handleId.bytes.toHex()}.pending").toPath()
        try {
    ...
    fun destroy(handleId: StableId): Boolean = file(handleId).delete()

    private fun file(handleId: StableId) = directory.resolve("${handleId.bytes.toHex()}.bin")

    private fun associatedData(handleId: StableId): ByteArray = "ledger-import-source-handle-v1\u0000".toByteArray(Charsets.US_ASCII) + handleId.bytes
